...
from ..utils.theano_utils import floatX
...
class Sequential(Layer):
    ...
    def add(self, layer,layer_mask=None):
        ...
        if layer_mask !=None:  #PAT added
            if layer_mask.shape != layer.params.eval()shape:
                raise Exception("Layer mask %s not compatible with layer shape %s." % (layer.params.eval().shape, w.shape))
            self.layer_masks += floatX(layer_mask)
        else:
            layer_masks += layer_mask
